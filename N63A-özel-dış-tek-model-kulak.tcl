...
for {set i 1} {$i <= $n} {incr i} {
    set fileout [open $name1$i.dat a]
    data $name1$i$specta
######################################################################################################
	ignore **-$Emin
	ignore $Emax-**
...
######################################################################################################
	...
	setplot command la t Model N63A spectrum: $name1$i
	setplot command la f chi-squared $chi1 / $dof1 dof

	plot ldata del
    cpd $name1$i-tekmodel-1
###################################################################################################
    thaw 25-28,32
    fit
    thaw 2
...
###################################################################################################
    ...
    setplot command la t Model 1 spectrum: $name1$i
    setplot command la f chi-squared $chi1 / $dof1 dof

    plot ldata del
    cpd $name1$i-tekmodel-2
###################################################################################################
    tclout stat
    ...
    puts $fileout "$name1$i [lindex $stat 0] 			--- chi-square"
#-------------------------------------------------------------------------------------------------
    tclout dof
    ...
    puts $fileout "$name1$i [lindex $dof 0] 				--- dof"
#-------------------------------------------------------------------------------------------------
    tclout param 2
    ...
    puts $fileout "$name1$i [lindex $lpar2 0] 			--- nH"
#-------------------------------------------------------------------------------------------------
    tclout param 20
    ...
    puts $fileout "$name1$i [lindex $lpar20 0] 			--- kT"
#-------------------------------------------------------------------------------------------------
    tclout param 25
    ...
    puts $fileout "$name1$i [lindex $lpar25 0] 			--- O"
#-------------------------------------------------------------------------------------------------
    tclout param 26
    ...
    puts $fileout "$name1$i [lindex $lpar26 0] 			--- Ne"
#-------------------------------------------------------------------------------------------------
    tclout param 27
    ...
    puts $fileout "$name1$i [lindex $lpar27 0] 			--- Mg"
#-------------------------------------------------------------------------------------------------
    tclout param 28
    ...
    puts $fileout "$name1$i [lindex $lpar28 0] 			--- Si"
#-------------------------------------------------------------------------------------------------
#        tclout param 29
...
#
#        puts $fileout "$name1$i [lindex $lpar29 0] 			--- S"
#-------------------------------------------------------------------------------------------------
    tclout param 32
    ...
    puts $fileout "$name1$i [lindex $lpar32 0] 			--- Fe"
#-------------------------------------------------------------------------------------------------
    tclout param 35
    ...
    puts $fileout "$name1$i [lindex $lpar35 0] 			--- Tau_U"
#-------------------------------------------------------------------------------------------------
    tclout param 37
    ...
    puts $fileout "$name1$i [lindex $lpar37 0] 			--- Norm \n" 
#################################################################################################
    #parallel error 9
...
#################################################################################################    
    ...
    puts $fileout "$name1$i [lindex $stat 0] 			rerror--- chi-square"
    #---------------------------------------------------------------------------------------------------
    tclout dof
    set dof [string trim $xspec_tclout]
    ...
    puts $fileout "$name1$i [lindex $dof 0] 			rerror	--- dof"
    #---------------------------------------------------------------------------------------------------
    tclout error 2
    set par2 [string trim $xspec_tclout]
    ...
    puts $fileout "$name1$i [lindex $lpar2 0] 			rerror --- nH - err"
    #---------------------------------------------------------------------------------------------------
    tclout error 2
    set par2 [string trim $xspec_tclout]
    ...
    puts $fileout "$name1$i [lindex $lpar2 1] 	rerror --- nH + err"
    #---------------------------------------------------------------------------------------------------
    tclout error 20
    set par20 [string trim $xspec_tclout]
    ...
    puts $fileout "$name1$i [lindex $lpar20 0] 		rerror --- kT - err"
    #---------------------------------------------------------------------------------------------------
    tclout error 20
    set par20 [string trim $xspec_tclout]
    ...
    puts $fileout "$name1$i [lindex $lpar20 1] 		rerror --- kT + err"
    #---------------------------------------------------------------------------------------------------
    tclout error 25
    set par25 [string trim $xspec_tclout]
    ...
    puts $fileout "$name1$i [lindex $lpar25 0] 	rerror --- O - err"
    #---------------------------------------------------------------------------------------------------
    tclout error 25
    set par25 [string trim $xspec_tclout]
    ...
    puts $fileout "$name1$i [lindex $lpar25 1] 		rerror --- O + err"
    #---------------------------------------------------------------------------------------------------
    tclout error 26
    set par26 [string trim $xspec_tclout]
    ...
    puts $fileout "$name1$i [lindex $lpar26 0] 		rerror --- Ne - err"
    #---------------------------------------------------------------------------------------------------
    tclout error 26
    set par26 [string trim $xspec_tclout]
    ...
    puts $fileout "$name1$i [lindex $lpar26 1] 		rerror --- Ne + err"
    #---------------------------------------------------------------------------------------------------
    tclout error 27
    set par27 [string trim $xspec_tclout]
    ...
    puts $fileout "$name1$i [lindex $lpar27 0] 		rerror --- Mg - err"
    #---------------------------------------------------------------------------------------------------
    tclout error 27
    set par27 [string trim $xspec_tclout]
    ...
    puts $fileout "$name1$i [lindex $lpar27 1] 		rerror --- Mg + err"
    #---------------------------------------------------------------------------------------------------
    tclout error 28
    set par28 [string trim $xspec_tclout]
    ...
    puts $fileout "$name1$i [lindex $lpar28 0] 		rerror --- Si - err"
    #---------------------------------------------------------------------------------------------------
    tclout error 28
    set par28 [string trim $xspec_tclout]
    ...
    puts $fileout "$name1$i [lindex $lpar28 1] 		rerror --- Si + err"
    #---------------------------------------------------------------------------------------------------
    #tclout error 29
    ...
    #puts $fileout "$name1$i [lindex $lpar29 0] 		rerror --- S - err"
    #---------------------------------------------------------------------------------------------------
    #tclout error 29
    #set par29 [string trim $xspec_tclout]
    ...
    #puts $fileout "$name1$i [lindex $lpar29 1] 		rerror --- S + err"
    #---------------------------------------------------------------------------------------------------
    tclout error 32
    set par32 [string trim $xspec_tclout]
    ...
    puts $fileout "$name1$i [lindex $lpar32 0] 		rerror --- Fe - err"
    #---------------------------------------------------------------------------------------------------
    tclout error 32
    set par32 [string trim $xspec_tclout]
    ...
    puts $fileout "$name1$i [lindex $lpar32 1] 		rerror --- Fe + err"
    #---------------------------------------------------------------------------------------------------
    tclout error 35
    set par35 [string trim $xspec_tclout]
    ...
    puts $fileout "$name1$i [lindex $lpar35 0] 	rerror --- Tau_u - err"
    #---------------------------------------------------------------------------------------------------
    tclout error 35
    set par35 [string trim $xspec_tclout]
    ...
    puts $fileout "$name1$i [lindex $lpar35 1] 	rerror --- Tau_u + err"
    #---------------------------------------------------------------------------------------------------
    tclout error 37
    set par37 [string trim $xspec_tclout]
    ...
    puts $fileout "$name1$i [lindex $lpar37 0] 	rerror --- Norm - err"
    #---------------------------------------------------------------------------------------------------
    tclout error 37
    set par37 [string trim $xspec_tclout]
    ...
    puts $fileout "$name1$i [lindex $lpar37 1] 	rerror --- Norm + err \n"
    #####################################################################################################    
    tclout stat 
    set chi1 $xspec_tclout
    tclout dof 
