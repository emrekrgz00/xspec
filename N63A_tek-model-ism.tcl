...
for {set i 3} {$i <= $n} {incr i} {
	cd $name-$i$b
	set fileout [open $name$i$nei.dat a]
	data $name-$i$specta
###################################################################################################        
    ignore **-$Emin
	ignore $Emax-**
...
##################################################################################################
	...
	newpar 1   0.065     	0.01  	1.0E-4  1.0E-4  10     	10    
...
##################################################################################################
	...
	set stat [split $stat]
	puts $fileout "$i [lindex $stat 0] 			--- chi-square"
#---------------------------------------------------------------------------------------------------
	tclout dof
	set dof [string trim $xspec_tclout]
	regsub -all { +} $dof { } dof
	set dof [split $dof]
	puts $fileout "$i [lindex $dof 0] 				--- dof"
#---------------------------------------------------------------------------------------------------
	tclout param 2
...
##################################################################################################	
	...
	regsub -all { +} $stat { } stat
	set stat [split $stat]
	puts $fileout "$i [lindex $stat 0] 			rerror--- chi-square"
#---------------------------------------------------------------------------------------------------
	tclout dof
	set dof [string trim $xspec_tclout]
	regsub -all { +} $dof { } dof
	set dof [split $dof]
	puts $fileout "$i [lindex $dof 0] 			rerror	--- dof"
#---------------------------------------------------------------------------------------------------
	tclout error 2
	set par2 [string trim $xspec_tclout]
	...
	puts $fileout "$i [lindex $lpar2 0] 			rerror --- nH - err"
#---------------------------------------------------------------------------------------------------
	tclout error 2
	set par2 [string trim $xspec_tclout]
	...
	puts $fileout "$i [lindex $lpar2 1] 	rerror --- nH + err"
#---------------------------------------------------------------------------------------------------
	tclout error 20
	set par20 [string trim $xspec_tclout]
	regsub -all { +} $par20 { } cpar20
	set lpar20 [split $cpar20]
	puts $fileout "$i [lindex $lpar20 0] 		rerror --- kT - err"
#---------------------------------------------------------------------------------------------------
	tclout error 20
	set par20 [string trim $xspec_tclout]
	regsub -all { +} $par20 { } cpar20
	set lpar20 [split $cpar20]
	puts $fileout "$i [lindex $lpar20 1] 		rerror --- kT + err"
#---------------------------------------------------------------------------------------------------
	tclout error 25
	set par25 [string trim $xspec_tclout]
	regsub -all { +} $par25 { } cpar25
	set lpar25 [split $cpar25]
	puts $fileout "$i [lindex $lpar25 0] 	rerror --- O - err"
#---------------------------------------------------------------------------------------------------
	tclout error 25
	set par25 [string trim $xspec_tclout]
	regsub -all { +} $par25 { } cpar25
	set lpar25 [split $cpar25]
	puts $fileout "$i [lindex $lpar25 1] 		rerror --- O + err"
#---------------------------------------------------------------------------------------------------
	tclout error 26
	set par26 [string trim $xspec_tclout]
	regsub -all { +} $par26 { } cpar26
	set lpar26 [split $cpar26]
	puts $fileout "$i [lindex $lpar26 0] 		rerror --- Ne - err"
#---------------------------------------------------------------------------------------------------
	tclout error 26
	set par26 [string trim $xspec_tclout]
	regsub -all { +} $par26 { } cpar26
	set lpar26 [split $cpar26]
	puts $fileout "$i [lindex $lpar26 1] 		rerror --- Ne + err"
#---------------------------------------------------------------------------------------------------
	tclout error 27
	set par27 [string trim $xspec_tclout]
	regsub -all { +} $par27 { } cpar27
	set lpar27 [split $cpar27]
	puts $fileout "$i [lindex $lpar27 0] 		rerror --- Mg - err"
#---------------------------------------------------------------------------------------------------
	tclout error 27
	set par27 [string trim $xspec_tclout]
	regsub -all { +} $par27 { } cpar27
	set lpar27 [split $cpar27]
	puts $fileout "$i [lindex $lpar27 1] 		rerror --- Mg + err"
#---------------------------------------------------------------------------------------------------
	tclout error 28
	set par28 [string trim $xspec_tclout]
	regsub -all { +} $par28 { } cpar28
	set lpar28 [split $cpar28]
	puts $fileout "$i [lindex $lpar28 0] 		rerror --- Si - err"
#---------------------------------------------------------------------------------------------------
	tclout error 28
	set par28 [string trim $xspec_tclout]
	regsub -all { +} $par28 { } cpar28
	set lpar28 [split $cpar28]
	puts $fileout "$i [lindex $lpar28 1] 		rerror --- Si + err"
#---------------------------------------------------------------------------------------------------
#	tclout error 29
#	set par29 [string trim $xspec_tclout]
...
#	puts $fileout "$i [lindex $lpar29 0] 		rerror --- S - err"
#---------------------------------------------------------------------------------------------------
#	tclout error 29
#	set par29 [string trim $xspec_tclout]
...
#	 puts $fileout "$i [lindex $lpar29 1] 		rerror --- S + err"
#---------------------------------------------------------------------------------------------------
	tclout error 32
	set par32 [string trim $xspec_tclout]
	regsub -all { +} $par32 { } cpar32
	set lpar32 [split $cpar32]
	puts $fileout "$i [lindex $lpar32 0] 		rerror --- Fe - err"
#---------------------------------------------------------------------------------------------------
	tclout error 32
	set par32 [string trim $xspec_tclout]
	regsub -all { +} $par32 { } cpar32
	set lpar32 [split $cpar32]
	puts $fileout "$i [lindex $lpar32 1] 		rerror --- Fe + err"
#---------------------------------------------------------------------------------------------------
	tclout error 35
	set par35 [string trim $xspec_tclout]
	regsub -all { +} $par35 { } cpar35
	set lpar35 [split $cpar35]
	puts $fileout "$i [lindex $lpar35 0] 	rerror --- Tau_u - err"
#---------------------------------------------------------------------------------------------------
	tclout error 35
	set par35 [string trim $xspec_tclout]
	regsub -all { +} $par35 { } cpar35
	set lpar35 [split $cpar35]
	puts $fileout "$i [lindex $lpar35 1] 	rerror --- Tau_u + err"
#---------------------------------------------------------------------------------------------------
	tclout error 37
	set par37 [string trim $xspec_tclout]
	regsub -all { +} $par37 { } cpar37
	set lpar37 [split $cpar37]
	puts $fileout "$i [lindex $lpar37 0] 	rerror --- Norm - err"
#---------------------------------------------------------------------------------------------------
	tclout error 37
	set par37 [string trim $xspec_tclout]
	regsub -all { +} $par37 { } cpar37
	set lpar37 [split $cpar37]
	puts $fileout "$i [lindex $lpar37 1] 	rerror --- Norm + err \n"
################################################################################################## 
#	cpd /xw
#    plot ldat del
#    iplot
...
csize 1.3
time off
LAbel 3 VP 0.80 0.84 "Kenar6" csize 2
label pos y 2.5
la T
la F
plot

hardcopy Kenar6.png/cps
quit
...
csize 1.3
time off
LAbel 3 VP 0.80 0.84 "ISM4" csize 2
label pos y 2.5
la T
la F
plot

hardcopy ISM.png/cps
quit
"""