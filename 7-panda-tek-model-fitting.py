...
for i in data:
    ...
    show_model(outfile="../../../output/"+str(i[:-2])+"_tek-model.txt", clobber=True)
    show_fit(outfile="../../../reduce-chi/"+str(i[:-2])+"_tek-model_chi_sq.txt", clobber=True)
    plot_fit_delchi(xlog=True ,ylog=True, yerrorbars=True, xerrorbars=True)
    plt.savefig('../../../figures/'+str(i[:-2])+'_tekmodel'+'.png', dpi=480)
